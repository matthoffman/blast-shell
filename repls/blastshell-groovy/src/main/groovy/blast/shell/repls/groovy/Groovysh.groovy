...
class Groovysh extends Shell {
  ...
  */
  ...
  int run(final Terminal term, final String commandLine) {

    if (log.debug) {
      log.debug("Terminal ($term)")
      log.debug("    Supported:  $term.supported")
      log.debug("    ECHO:       (enabled: $term.echoEnabled)")
      log.debug("    H x W:      $term.height x $term.width")
      log.debug("    ANSI:       ${term.ansiSupported}")
    ...
    try {
      ...
      if (commandLine != null && commandLine.trim().size() > 0) {
        ...
        // Setup the history
        def file = new File(userStateDirectory, 'groovysh.history')
        runner.historyFile = file
        runner.history = history = new FileHistory(file)
